...
char * uriDecodeString( char *str ) {
  ...
  while ( str[inpos] ) {
    ...
    if ( str[inpos] == '%' && IS_HEX(str[inpos+1]) && IS_HEX(str[inpos+2]) ) {

      int c = 0;

      char hex1 = str[inpos+1];
...
static int _uri_parse_userinfo( struct uri *uri, const char **pos, const char *end ) {
  ...
  *pos = p+1; // skips '@'
  ...
  if ( uri->opts & URI_OPT_DECODE_USERINFO && uri->userinfo ) {
    uriDecodeString( uri->userinfo );
  }

  if ( uri->opts & URI_OPT_DECODE_USERPASS ) {
    if ( uri->user )
      uriDecodeString( uri->user );
    if ( uri->pass )
      uriDecodeString( uri->pass );
  }
...
static int _uri_parse_hostname( struct uri *uri, const char **pos, const char *end ) {
  ...
  if ( **pos == '[' ) {
    ...
    char *iperr = NULL;
    int rc = 0;
    if ( (rc = isIPv6( uri->host, strlen(uri->host), &iperr )) ) {
  ...
  if ( !(uri->flags & URI_T_HOST_IPV6) && uri->host != NULL && *uri->host != 0x00 ) {
    ...
    if ( isIPv4( uri->host, strlen(uri->host), NULL ) ) {
...
static void _uri_scheme2schemeType( struct uri *uri ) {
  if      ( strcmp( uri->scheme, "afp"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_AFP;    }
  else if ( strcmp( uri->scheme, "cifs"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_CIFS;   }
  else if ( strcmp( uri->scheme, "data"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_DATA;   }
  else if ( strcmp( uri->scheme, "dns"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_DNS;    }
  else if ( strcmp( uri->scheme, "file"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_FILE;   }
  else if ( strcmp( uri->scheme, "ftp"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_FTP;    }
  else if ( strcmp( uri->scheme, "http"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_HTTP;   }
  else if ( strcmp( uri->scheme, "https"  ) == 0 ) { uri->scheme_t = URI_SCHEME_T_HTTPS;  }
  else if ( strcmp( uri->scheme, "imap"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_IMAP;   }
  else if ( strcmp( uri->scheme, "irc"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_IRC;    }
  else if ( strcmp( uri->scheme, "mailto" ) == 0 ) { uri->scheme_t = URI_SCHEME_T_MAILTO; }
  else if ( strcmp( uri->scheme, "nfs"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_NFS;    }
  else if ( strcmp( uri->scheme, "pop"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_POP;    }
  else if ( strcmp( uri->scheme, "rtsp"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_RTSP;   }
  else if ( strcmp( uri->scheme, "sftp"   ) == 0 ) { uri->scheme_t = URI_SCHEME_T_SFTP;   }
  else if ( strcmp( uri->scheme, "sip"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_SIP;    }
  else if ( strcmp( uri->scheme, "smb"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_SMB;    }
  else if ( strcmp( uri->scheme, "ssh"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_SSH;    }
  else if ( strcmp( uri->scheme, "tel"    ) == 0 ) { uri->scheme_t = URI_SCHEME_T_TEL;    }
  else if ( strcmp( uri->scheme, "telnet" ) == 0 ) { uri->scheme_t = URI_SCHEME_T_TELNET; }
  else { uri->scheme_t = URI_SCHEME_T_UNKNOWN; }
}



// static void uri_slash2slashType( struct uri *uri ) {
//   if      (         uri->slash[0] == 0x00      ) { uri->slash_t = URI_SLASH_T_NO_SLASH; }
//   else if ( strcmp( uri->slash, "/"     ) == 0 ) { uri->slash_t = URI_SLASH_T_1_SLASH;  }
//   else if ( strcmp( uri->slash, "//"    ) == 0 ) { uri->slash_t = URI_SLASH_T_2_SLASH;  }
//   else if ( strcmp( uri->slash, "///"   ) == 0 ) { uri->slash_t = URI_SLASH_T_3_SLASH;  }
//   else if ( strcmp( uri->slash, "////"  ) == 0 ) { uri->slash_t = URI_SLASH_T_4_SLASH;  }
//   else if ( strcmp( uri->slash, "/////" ) == 0 ) { uri->slash_t = URI_SLASH_T_5_SLASH;  }
//   else if ( strcmp( uri->slash, "//./"  ) == 0 ) { uri->slash_t = URI_SLASH_T_DOS_DEV_DOT; }
//   else if ( strcmp( uri->slash, "//?/"  ) == 0 ) { uri->slash_t = URI_SLASH_T_DOS_DEV_QUESTION_MARK; }
//   else { uri->slash_t = URI_SLASH_T_UNKNOWN; }
// }
